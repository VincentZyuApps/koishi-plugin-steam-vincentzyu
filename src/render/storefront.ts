...
export async function renderStorefront(ctx: Context, config: Config, sections: StorefrontSection[]) {
  ...
  const images = await resolveSteamImages(ctx, config, shown.flatMap(section => section.items.map(item => item.image)))
  const content = pageTemplate('storefront', {
    SECTIONS: shown.map(section => partialTemplate('storefront-section', {
      ...
      ITEMS: section.items.map((item, index) => partialTemplate('storefront-item', {
        RANK: String(index + 1),
        IMAGE: steamImage(item.image ? images.get(item.image) : undefined, item.name),
