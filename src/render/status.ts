...
export async function renderStatus(ctx: Context, config: Config, profile: SteamStatusProfile) {
  const gameImage = profile.gameId ? headerImage(profile.gameId) : undefined
  const images = await resolveSteamImages(ctx, config, [profile.background, profile.avatar, profile.frame, gameImage])
  const asset = (url: string | undefined, className: string, alt: string, placeholder = '') => {
    const source = url ? images.get(url) : undefined
  ...
  return renderPage(ctx, config, { title: '👤 Steam 状态', subtitle: '🌐 公开个人资料', content, fontText, source: 'Steam Web API / Steam Community' })
}
