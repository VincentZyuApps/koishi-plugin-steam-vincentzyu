...
export async function renderRanks(ctx: Context, config: Config, title: string, ranks: RankEntry[]) {
  const shown = ranks.slice(0, config.rankingLimit)
  const images = await resolveSteamImages(ctx, config, shown.map(item => item.image))
